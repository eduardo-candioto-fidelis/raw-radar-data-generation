...
# Loading raw data:
raw_data = np.load(f'./data/generated/{name}.npy')
#raw_data = np.load(f'./data/preprocessed/{name}.npy')
...
for i in tqdm(range(0, len(raw_data))):
    ...
    for detection in detections:
        distance = radar._DScale[detection[1]]
        azimuth = radar._AScale[detection[0]]
        labels.append([float(distance), float(azimuth)])
...
np.save(f'data/labels/{name}_labels.npy', np.array(labels))
